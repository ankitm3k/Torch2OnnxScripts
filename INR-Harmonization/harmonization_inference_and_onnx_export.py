...
import math
import torch.onnx.symbolic_opset16 as sym_opset16  

def symbolic_unfold(g, input, dim, size, *args):
    # Implement symbolic operation for aten::unfold
    axes_node = g.op("Constant", value_t=torch.tensor([dim]).long())
    result = g.op("Unsqueeze", input, axes_i=[axes_node])
    result = g.op("Slice", result, starts_i=[0], ends_i=[1], axes_i=[dim])
    result = g.op("Unsqueeze", result, axes_i=[dim])
    result = g.op("Reshape", input, result, shape_i=[sym_opset16.size_opset16(input, g.op("Constant", value_t=torch.tensor(size).long())), -1])

    return result

# Register the symbolic function for aten::unfold
torch.onnx.register_custom_op_symbolic('aten::unfold', symbolic_unfold, 16) 
...
def inference(model, opt, composite_image=None, mask=None):
    ...
    print("ENTER ONNX EXPORT!!!\n")
    onnx_path = "harmonization_model.onnx"
    torch.onnx.export(model,
                  (composite_image, mask,fg_INR_coordinates,start_proportion[0]),
                  onnx_path,
                  verbose=True,
                  opset_version=16,
                  operator_export_type=torch.onnx.OperatorExportTypes.ONNX_ATEN_FALLBACK,
                  input_names=['composite_image', 'mask','fg_INR_coordinates','start_proportion'],
                  output_names=['output'],
                  dynamic_axes={'composite_image': {0: 'batch_size',1: 'img_channel',2: 'img_height',3: 'img_width'},
                                'mask': {0: 'batch_size',1: 'mask_channel',2: 'mask_height',3: 'mask_width'},
                                'output': {0: 'batch_size',1: 'out_channel',2: 'out_height',3: 'out_width'}})
...
def main_process(opt, composite_image=None, mask=None):
    ...
    load_dict = torch.load(opt.pretrained, map_location=opt.device)

    model.load_state_dict(load_dict, strict=False)

    inference(model, opt, composite_image, mask)
